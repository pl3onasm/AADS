...
//=================================================================
// Adds an edge to the network by label
...
network *copynetwork(network *N) {
  if (! N) return NULL;
  network *copy = newNetwork(nVertices(N));

  for (edge *e = firstE(N); e; e = nextE(N)) 
    addVandE(copy, e->from->label, e->to->label, e->cap);
  return copy;
}
