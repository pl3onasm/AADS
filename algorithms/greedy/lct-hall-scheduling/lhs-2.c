/* 
  ...
  note: this version uses a stack to keep track of available halls,
        which will schedule activities differently than the
        lhs-3.c version, which uses a binary heap, but the
        schedules are, of course, still optimal
*/ 
...
typedef struct hall {
  ...
  size_t schSize;           // number of events in the schedule
} hall;
  ...
  for (size_t i = 0; i < nActs * 2; i++) {
    ...
    if (e.type == START) {
      ...
      e.act->hallId = h->id;
        ...
        // if an activity ends, the hall becomes available
